...
@rijndael_cmd_exec = (
    ### command execution tests
    {
        ...
        'cmdline'  => qq|$fwknopCmd --server-cmd "touch $cmd_exec_test_file" | .
            "$client_sdp_options -a $fake_ip -D $loopback_ip --get-key $local_key_file " .
            "$verbose_str",
        'fwknopd_cmdline'  => "$fwknopdCmd $srv_sdp_options -c $cf{'def'} -a $cf{'cmd_access'} " .
    ...
    },
    {
        ...
        'fwknopd_cmdline'  => "$fwknopdCmd $srv_sdp_options -c $cf{'def'} -a $cf{'cmd_setuid_access'} " .
            "-d $default_digest_file -p $default_pid_file $intf_str",
        'fw_rule_created' => $REQUIRE_NO_NEW_RULE,
    },
    {
        ...
        'cmdline'  => qq|$fwknopCmd --server-cmd "touch $cmd_exec_test_file" | .
            ...
            "$verbose_str",
        'cmd_exec_file_owner' => 'nobody',
        'fwknopd_cmdline'  => "$fwknopdCmd $srv_sdp_options -c $cf{'def'} -a $cf{'cmd_giduid_access'} " .
            "-d $default_digest_file -p $default_pid_file $intf_str",
        'fw_rule_created' => $REQUIRE_NO_NEW_RULE,
    },
